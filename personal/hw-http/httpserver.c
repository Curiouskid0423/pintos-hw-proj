...
void serve_file(int fd, char* path) {
  ...
  stat(path, sb);
  char len_buf[10];
  int convert_ret = snprintf(len_buf, 10, "%d", sb->st_size);
  http_send_header(fd, "Content-Length", len_buf); // TODO: change this line too
  http_end_headers(fd);
  int input_fd = open(path, O_RDONLY);
  int size_read = sendfile(fd, input_fd, NULL, sb->st_size);
  if (size_read == -1)
    printf("Failed to use sendfile.");
  free(sb);
  /* PART 2 END */
}
...
void serve_directory(int fd, char* path) {
  ...
  http_format_index(buff, path);
  if (access(buff, R_OK) != -1) {
    serve_file(fd, buff);
  } else {
    http_start_response(fd, 200);
    http_send_header(fd, "Content-Type", http_get_mime_type(buff));
    http_end_headers(fd);
    DIR* ptr = opendir(path);
    struct dirent* entry;
    while ((entry = readdir(ptr)) != NULL) {
      char fname_buff[256];
      http_format_href(fname_buff, path, entry->d_name);
      write(fd, fname_buff, 512); // TO_FIX: Not checking the number of written bytes yet.
    }
  }
...
void handle_files_request(int fd) {
  ...
  if (access(path, R_OK) != -1) {
    ...
    http_send_header(fd, "Content-Type", "404 Error response");
    http_end_headers(fd);
  }
...
void* proxy_helper(void* wrapper) {
  proxy_args_t* args = (proxy_args_t*)wrapper;
  char buff[1000];
  while (1) {
    memset(&buff, '\0', sizeof(buff));
    int read_bytes = read(args->in_fd, buff, sizeof(buff));
    if (read_bytes < 0) {
      return NULL;
    } else if (read_bytes > 0) {
      int write_bytes = write(args->out_fd, buff, read_bytes);
      if (write_bytes < 0)
        return NULL;
    }
  }
...
void handle_proxy_request(int fd) {
  ...
  */
  ...
  proxy_args_t* cli_args = malloc(sizeof(proxy_args_t));
  proxy_args_t* tgt_args = malloc(sizeof(proxy_args_t));

  cli_args->in_fd = fd; // Write from client to target;
  cli_args->out_fd = target_fd;

  tgt_args->in_fd = target_fd; // Write from target to client
  tgt_args->out_fd = fd;

  pthread_t thread1, thread2;
  pthread_create(&thread1, NULL, proxy_helper, (void*)cli_args);
  pthread_create(&thread2, NULL, proxy_helper, (void*)tgt_args);
  pthread_join(thread1, NULL);
  pthread_join(thread2, NULL);

  // Why does freeing memory break the code?
  // free(cli_args);
  // free(tgt_args);
  close(fd);
  close(target_fd);
...
void init_thread_pool(int num_threads, void (*request_handler)(int)) {
  ...
  for (int i = 0; i < num_threads; i += 1) {
    pthread_create(&new_threads[i], NULL, handle_clients, (void*)request_handler);
  }
  /* PART 7 END */
}
#endif
...
void serve_forever(int* socket_number, void (*request_handler)(int)) {
  ...
  *socket_number = socket(PF_INET, SOCK_STREAM, 0);
  ...
  int bind_ret = bind(*socket_number, (struct sockaddr*)&server_address, sizeof(server_address));
  if (bind_ret == -1) {
    printf("binding failed.");
    return;
  }
  int LISTEN_BACKLOG = 1024; // default to 1024.
  listen(*socket_number, LISTEN_BACKLOG);
  /* PART 1 END */
  ...
  while (1) {
    ...
    if (pid == 0) {
      ...
      int BACKLOG = 1024;
      int status;
      listen(client_socket_number, BACKLOG);
      wait(&status);
    ...
    pthread_detach(pthread_self());
    pthread_create(&new_thread, NULL, request_handler, client_socket_number);
    listen(client_socket_number, 1024);
    /* PART 6 END */
#elif POOLSERVER
    /*
     * TODO: PART 7
